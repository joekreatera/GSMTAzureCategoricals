...
class SSLearnerLeaf(BaseEstimator, ClassifierMixin):
    ...
    def import_model(self, node_info, ):
        '''
        node_info contains a dataframe of 1 rows with the information, by each label, of where the model is. 
        If the information is just a number, then it is a constant. 
        '''
        ...
        for col in node_info.columns:
            if col.find("model_") >= 0:
                # print("doing column : " , col )
                label = col.replace("model_", "")
                self.columns.append( label )
                v = node_info[col].values[0]
                self.thresholds.append( eval(node_info[ f"threshold_{label}" ].astype("str").values[0] )  )
                try:
                    v = float(v)
                    self.models.append( int(v) )
                except ValueError:
                    with open(node_info[col].values[0], "rb") as f:
                        self.models.append( load(f) ) 
                        #print(self.models)

        self.output_features_ = len(self.columns)
        #print( node_info[ ["tree_id","node_id"] ] , " :: " , self.models  )
        #print( node_info[ ["tree_id","node_id"] ] , " :: " , self.thresholds  )
    ...
    def export_model(self, export_folder = "" ,  tree_id = -1 , node_id = -1 ):
        i = 0
        models_export_details=[]
        for label in self.columns:
            nm = f"{export_folder}/model_{tree_id}_{node_id}_{label}"

            if( type(self.models[i]) is KNeighborsClassifier ):
                with open(nm+"_knn.pkl", "wb") as f:
                    dump(self.models[i], f, protocol=HIGHEST_PROTOCOL)
                models_export_details.append(nm+"_knn.pkl")
            elif( type(self.models[i]) is RandomForestClassifier ):
                with open(nm+"_rf.pkl", "wb") as f:
                    dump(self.models[i], f, protocol=HIGHEST_PROTOCOL)
                models_export_details.append(nm+"_rf.pkl")
            else:
                models_export_details.append(self.models[i])
            i += 1
        return models_export_details
    ...
    def predict_with_proba(self, X, optional_global_labels_distribution = None, y_true = None):
        ...
        probabilities = []
        for label in self.columns:
            # print(f"predicting X on {label} " , type(self.models[i])  , " ===> ",  type(self.models[i]) is RandomForestClassifier  )
            if( type(self.models[i]) is KNeighborsClassifier ):
                ...
                # print("trying with " , x ," for " , getattr(self.models[i],"feature_names_in_") )
                try:
                    prob = self.models[i].predict_proba(x)
